...
describe InvitationsMailer do
  describe "invite" do
    let(:invite) { mock_model Invitation, :account_name => 'Apple', :email => 'rs@example.org', :receiver_name => 'Radoslav Stankov', :token => 'foo' }
    ...
    it { should have_body_text %r(Accept invitation)}
  end
end
